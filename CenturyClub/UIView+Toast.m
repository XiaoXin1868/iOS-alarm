...
{
    [[[UIApplication sharedApplication].delegate window] makeToast:message duration:duration position:position];
}

+ (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title {
    [[[UIApplication sharedApplication].delegate window] makeToast:message duration:duration position:position title:title];
}

+ (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position image:(UIImage *)image
{
    [[[UIApplication sharedApplication].delegate window] makeToast:message duration:duration position:position image:image];
}
...
+ (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image {
    [[[UIApplication sharedApplication].delegate window] makeToast:message duration:duration position:position title:title image:image];
}
...
{
    [[[UIApplication sharedApplication].delegate window] makeToastActivity:position];
}
...
{
    [[[UIApplication sharedApplication].delegate window] showToast:toast];
}

+ (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point
{
    [[[UIApplication sharedApplication].delegate window] showToast:toast duration:duration position:point];
    }
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame)
        {
            ...
            switch (orientation) {
                ...
                {
                    CGAffineTransform tranform = toast.transform;
                    tranform = CGAffineTransformRotate(tranform,M_PI + M_PI_2);
                    toast.transform = tranform;
                    return CGPointMake(( (toast.frame.size.width / 2)) + CSToastVerticalPadding,self.bounds.size.height/2 );
                }
                ...
                case UIInterfaceOrientationLandscapeRight:
                {
                    ...
                    toast.transform = tranform;
                    return CGPointMake((self.bounds.size.width - (toast.frame.size.width / 2)) - CSToastVerticalPadding,self.bounds.size.height /2  );
                ...
                {
                    CGAffineTransform tranform = toast.transform;
                    tranform = CGAffineTransformRotate(tranform, M_PI);
                    toast.transform = tranform;
        ...
        else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame)
        {
            switch (orientation) {
                ...
                case UIInterfaceOrientationPortraitUpsideDown:
                {
                    ...
                    tranform = CGAffineTransformRotate(tranform, M_PI);
                    toast.transform = tranform;
                    return CGPointMake(self.bounds.size.width / 2,self.bounds.size.height - self.bounds.size.height / 2);
                }
                case UIInterfaceOrientationUnknown:
                    break;
...
- (CGSize)sizeForString:(NSString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    return [self sizeForText:string ofFont:font constraintTo:constrainedSize];
}
...
- (CGSize) sizeForText : (NSString *) text ofFont : (UIFont *) font constraintTo : (CGSize) size {
    CGRect frame = [text boundingRectWithSize:size
                                   ...
                                   attributes:@{NSFontAttributeName:font}
                                      context:nil];
//    NSLog(@"text.frame = %@", NSStringFromCGRect(frame));
    return frame.size;
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        titleLabel.font = [UIFont fontWithName:@"Avenir-Black" size:CSToastFontSize];//[UIFont boldSystemFontOfSize:CSToastFontSize];
    ...
    if (message != nil) {
        ...
        messageLabel.font = [UIFont fontWithName:@"Avenir-Medium" size:CSToastFontSize];//[UIFont systemFontOfSize:CSToastFontSize];
